...
//
// Only explicitly owned errors affect the shared retry ledger. Existing
// failover loops can call these helpers without counting their legacy retries.

// noteOpenAIUpstream5xxRetry 在同账号重试即将发生时记录一条「已拦截，准备重试」。
//
// 调用位置：handler 同账号重试分支中，sameAccountRetryCount 自增之后、
// time.After 等待之前。只登记待执行项，真正开始下次转发时才计数。
...
func markOpenAIUpstream5xxRetryCompleted(
	...
	service.SnapshotOpenAIUpstream5xxUsageRetries(c, result, false)
	// WS business recovery is timestamped at its first committed output,
	// independently of the network-first-frame metric retained in the result.
	if service.OpenAIWSBusinessRetryActive(c) {
		return
	}
