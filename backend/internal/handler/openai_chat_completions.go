			...
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					if c.Writer.Size() != writerSizeBeforeForward {
						...
						// 开关关闭时为 no-op。
						recordOpenAIUpstream5xxRetrySkipped(c, account, failoverErr, reqModel,
					...
					if failoverErr.RetryableOnSameAccount {
						...
						if sameAccountRetryAllowed(failoverErr, sameAccountRetryCount[account.ID], retryLimit) {
							...
							noteOpenAIUpstream5xxRetry(c, account, failoverErr, reqModel,
					...
					// 按正常错误返回客户端。开关关闭时恒为 false，沿用下方原有预算判定。
				...
				if !upstreamErrorAlreadyCommunicated {
					...
					if !wroteFallback {
						// 二次开发：传入 err，让 WS 链路的上游原文替代兜底文案。
						// CC 入站在强制 WS 下同样走 WS 上游（见
						// openai_ws_chat_completions_bridge.go），此前这里传 nil，
						// 客户端只能看到 "Upstream request failed"。
