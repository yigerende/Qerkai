...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		downstreamModel             sql.NullString
		stateInjected               sql.NullBool
		openAIUpstream5xxRetryCount sql.NullInt64
	...
	if downstreamModel.Valid {
		log.DownstreamModel = &downstreamModel.String
	}
	if stateInjected.Valid {
		log.StateInjected = &stateInjected.Bool
	}
	if openAIUpstream5xxRetryCount.Valid {
		value := int(openAIUpstream5xxRetryCount.Int64)
		log.OpenAIUpstream5xxRetryCount = &value
	}
	if groupID.Valid {
		value := groupID.Int64
		log.GroupID = &value
