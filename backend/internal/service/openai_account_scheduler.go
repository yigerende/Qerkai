...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	if previousResponseID != "" && NormalizeOpenAICompatiblePlatform(req.Platform) == PlatformOpenAI &&
		...
		if selection != nil && selection.Account != nil {
			...
			if !groupCompatible ||
				!compatible || !s.isAccountTransportCompatible(selection.Account, req.RequiredTransport, derefGroupID(req.GroupID)) {
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if !s.isAccountTransportCompatible(account, req.RequiredTransport, derefGroupID(req.GroupID)) {
		clearBinding()
		return nil, false, nil
	}
	account = s.service.recheckSelectedOpenAIAccountFromDB(ctx, account, req.GroupID, req.Platform, req.RequestedModel, req.RequireCompact, req.RequiredCapability)
	if account == nil || !s.service.openAIAccountMatchesSchedulingGroup(account, req.GroupID) || !s.isAccountRequestCompatible(ctx, account, req) || !s.isAccountTransportCompatible(account, req.RequiredTransport, derefGroupID(req.GroupID)) {
		clearBinding()
		return nil, false, nil
	}
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrderWithBudget(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport, derefGroupID(req.GroupID)) || !s.isAccountRequestCompatible(ctx, fresh, req) {
			release(result)
			continue
		}
...
func (s *defaultOpenAIAccountScheduler) tryFallbackToWeightedSticky(
	...
	for _, accountID := range []int64{req.StickyPreviousAccountID, req.StickyAccountID} {
		...
		if !s.isAccountRequestCompatible(ctx, account, req) || !s.isAccountTransportCompatible(account, req.RequiredTransport, derefGroupID(req.GroupID)) {
			continue
		}
		account = s.service.recheckSelectedOpenAIAccountFromDB(ctx, account, req.GroupID, req.Platform, req.RequestedModel, req.RequireCompact, req.RequiredCapability)
		if account == nil {
...
func (s *defaultOpenAIAccountScheduler) isAccountTransportCompatible(account *Account, requiredTransport OpenAIUpstreamTransport, groupIDs ...int64) bool {
	...
	return s.service.isOpenAIAccountTransportCompatible(account, requiredTransport, groupIDs...)
}
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	if scheduler == nil {
		...
		for {
			...
			if s.isOpenAIAccountTransportCompatible(selection.Account, requiredTransport, derefGroupID(groupID)) &&
...
func (s *OpenAIGatewayService) isOpenAIAccountTransportCompatible(account *Account, requiredTransport OpenAIUpstreamTransport, groupIDs ...int64) bool {
	...
	if requiredTransport == OpenAIUpstreamTransportResponsesWebsocketV2Ingress {
		if s.cfg == nil || !s.cfg.Gateway.OpenAIWS.ModeRouterV2Enabled {
			return resolveOpenAIWSProtocolForGroup(s.getOpenAIWSProtocolResolver(), account, firstForceWSGroupID(groupIDs)).Transport == OpenAIUpstreamTransportResponsesWebsocketV2
		}
	...
	return resolveOpenAIWSProtocolForGroup(s.getOpenAIWSProtocolResolver(), account, firstForceWSGroupID(groupIDs)).Transport == requiredTransport
}
...
func (s *OpenAIGatewayService) ReportOpenAIAccountScheduleResult(account *Account, model string, success bool, firstTokenMs *int, observedErr ...error) bool {
	...
	if success {
		...
		// 二次开发：该 (账号, 模型) 成功过就解除无权限黑名单，账号升级订阅后
		// 不必等 1 小时冷却自然到期。详见 openai_model_access_denied.go。
		clearOpenAIModelAccessDenied(accountID, model)
	}
