...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	wsGroupID := getOpenAIGroupIDFromContext(c)
	wsDecision := resolveOpenAIWSProtocolForGroup(s.getOpenAIWSProtocolResolver(), account, wsGroupID)
	// 仅允许 WS 入站请求走 WS 上游，避免出现 HTTP -> WS 协议混用。
	wsDecision = resolveOpenAIWSDecisionByClientTransport(wsDecision, GetOpenAIClientTransport(c), wsGroupID)
	...
	wsRetryLoop:
		for attempt := 1; attempt <= maxAttempts; attempt++ {
			...
			var businessRetryErr *UpstreamFailoverError
			...
			// 二次开发：池内陈旧连接失败不计入重试预算与 attempt。
			// 该连接在池中空闲期间被上游 keepalive 打死（coder/websocket 无
			// reader 时不回 pong），与本次请求的上游状态无关——那次「重试」
			// 才是真正的第一次尝试。每请求只放行一次。
			// 详见 openai_ws_stale_conn.go。
			...
			reason, retryable := classifyOpenAIWSReconnectReasonWithStale(wsErr)
			if reason != "" {
		...
		// 二次开发：帧超限时回退 HTTP，避免 HTTP 客户端因代理内部选择 WS 上游而收到 413。
		// 详见 openai_force_upstream_ws.go。
		if shouldFallbackToHTTPAfterWSFailure(c, wsErr) {
			logOpenAIWSHTTPFallback(account, wsErr)
			// WS 分支的重连恢复会就地修改 ensureReqBody() 返回的 map
			// （如 delete previous_response_id）。回退前丢弃该缓存，
			...
			if !s.writeOpenAIWSFallbackErrorResponse(c, account, wsErr) {
				...
				// 后被硬编码成 "Upstream request failed"，真实原因整体丢失
				// （生产实测占客户端可见错误 22.4%）。
				// 详见 openai_ws_fallback_client_error.go。
				return nil, s.wrapOpenAIWSFallbackClientError(c, account, wsErr)
			}
			return nil, wsErr
		}
	}
