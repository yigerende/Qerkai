...
func TestAccountQualityPostgresPersistenceConcurrencyAndMigration(t *testing.T) {
	...
	_, err = db.Exec(`CREATE TABLE accounts(id BIGINT PRIMARY KEY, platform TEXT NOT NULL,deleted_at TIMESTAMPTZ);CREATE TABLE settings(key TEXT PRIMARY KEY,value TEXT NOT NULL)`)
	require.NoError(t, err)
