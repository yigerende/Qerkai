...
func NewOpenAIWSProtocolResolver(cfg *config.Config) OpenAIWSProtocolResolver {
	// 二次开发：包一层强制上游 WS 装饰器，使 Resolve 的全部调用点（HTTP 转发、
	// 账号调度、WS ingress、response-id 绑定）行为一致。开关关闭时为透明包装。
	// 详见 openai_force_upstream_ws.go。
	return newForceUpstreamWSProtocolResolver(&defaultOpenAIWSProtocolResolver{cfg: cfg})
}
