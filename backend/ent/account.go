...
type Account struct {
	...
	// SchedulingPausedAt holds the value of the "scheduling_paused_at" field.
	SchedulingPausedAt *time.Time `json:"scheduling_paused_at,omitempty"`
...
func (*Account) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case account.FieldCreatedAt, account.FieldUpdatedAt, account.FieldDeletedAt, account.FieldLastUsedAt, account.FieldExpiresAt, account.FieldSchedulingPausedAt, account.FieldRateLimitedAt, account.FieldRateLimitResetAt, account.FieldOverloadUntil, account.FieldTempUnschedulableUntil, account.FieldSessionWindowStart, account.FieldSessionWindowEnd:
...
func (_m *Account) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case account.FieldSchedulingPausedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field scheduling_paused_at", values[i])
			} else if value.Valid {
				_m.SchedulingPausedAt = new(time.Time)
				*_m.SchedulingPausedAt = value.Time
			}
...
func (_m *Account) String() string {
	...
	builder.WriteString(", ")
	if v := _m.SchedulingPausedAt; v != nil {
		builder.WriteString("scheduling_paused_at=")
		builder.WriteString(v.Format(time.ANSIC))
	}
	builder.WriteString(", ")
