<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    <Select :model-value="filters.quality_status" class="w-40" :options="qualityOpts" :aria-label="t('admin.accounts.qualityFilter.all')" @update:model-value="updateQualityStatus" @change="$emit('change')" />
...
const updateQualityStatus = (value: string | number | boolean | null) => { emit('update:filters', { ...props.filters, quality_status: value }) }
...
const qualityOpts = computed(() => [
  { value: '', label: t('admin.accounts.qualityFilter.all') },
  { value: 'degraded', label: t('admin.accounts.qualityFilter.degraded') },
  { value: 'normal', label: t('admin.accounts.qualityFilter.normal') },
  { value: 'pending', label: t('admin.accounts.qualityFilter.pending') }
])
