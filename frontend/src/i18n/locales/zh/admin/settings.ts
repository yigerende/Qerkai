export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        downstreamModelAlignment: 'OpenAI 下游模型名称对齐',
        downstreamModelAlignmentHint: '对选定分组和模型，当上游返回的模型名称与实际发送给上游的 model 不一致时，将下游响应中的模型名称改为实际发送给上游的 model。真实上游模型仍用于日志和降智检测；关闭后沿用原有处理。',
        downstreamModelAlignmentModels: '启用的模型',
        downstreamModelAlignmentModelsHint: '每行一个模型，按实际发送给上游的模型匹配；空列表不启用。支持 HTTP/SSE 和 WebSocket，修改名称不会改变回答内容或实际执行模型。',
        forceWSGroups: '生效分组',
