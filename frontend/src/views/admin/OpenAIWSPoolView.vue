              ...
              <template v-for="account in snapshot?.accounts || []" :key="account.account_id">
                ...
                <tr v-if="expanded.has(account.account_id)"><td colspan="9" class="bg-gray-50 px-5 py-4 dark:bg-dark-900/30">
                  <div class="overflow-x-auto rounded border border-gray-200 dark:border-dark-700">
                    <table class="min-w-full text-xs"><thead class="bg-white text-gray-500 dark:bg-dark-800"><tr><th class="px-3 py-2 text-left">{{ t('admin.openaiWSPool.columns.connectionId') }}</th><th class="px-3 py-2 text-left">{{ t('admin.openaiWSPool.columns.role') }}</th><th class="px-3 py-2 text-left">{{ t('admin.openaiWSPool.columns.sessionId') }}</th><th class="px-3 py-2 text-left">{{ t('admin.openaiWSPool.columns.status') }}</th><th class="px-3 py-2 text-left">{{ t('admin.openaiWSPool.columns.age') }}</th><th class="px-3 py-2 text-left">{{ t('admin.openaiWSPool.columns.idle') }}</th><th class="px-3 py-2 text-left">{{ t('admin.openaiWSPool.columns.waiters') }}</th></tr></thead>
                      <tbody class="divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-800"><tr v-for="conn in account.connections" :key="conn.id"><td class="px-3 py-2 font-mono text-gray-700 dark:text-gray-200">{{ conn.id }}</td><td class="px-3 py-2">{{ roleText(conn.role) }}</td><td class="px-3 py-2 font-mono text-gray-500">{{ conn.session || '-' }}</td><td class="px-3 py-2"><span :class="conn.state === 'in_use' ? 'text-emerald-600' : 'text-gray-500'">{{ t(`admin.openaiWSPool.connectionState.${conn.state === 'in_use' ? 'inUse' : 'idle'}`) }}</span></td><td class="px-3 py-2 tabular-nums">{{ duration(conn.age_seconds) }}</td><td class="px-3 py-2 tabular-nums">{{ duration(conn.idle_seconds) }}</td><td class="px-3 py-2 tabular-nums">{{ conn.waiters }}</td></tr><tr v-if="!account.connections.length"><td colspan="7" class="px-3 py-6 text-center text-gray-400">{{ t('admin.openaiWSPool.noConnections') }}</td></tr></tbody>
                    </table>
                  </div>
                </td></tr>
              </template>
