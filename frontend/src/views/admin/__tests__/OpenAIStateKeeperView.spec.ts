...
describe('Upstream state management', () => {
  ...
  it('saves the State scheduling requirement and its model and validity without starting collection', async () => {
    const data = initial()
    Object.assign(data.settings, { require_valid_state: false, scheduling_state_model: 'gpt-6-astra', scheduling_state_minutes: 55 })
    ...
    const toggle = w.get<HTMLInputElement>('[aria-label="调度必须有有效 State"]')
    ...
    await w.get('[aria-label="State 调度有效期（分钟）"]').setValue(50)
    await w.findAll('button').find(b => b.text() === '保存配置')!.trigger('click'); await flushPromises()
    expect(stateKeeperAPI.save).toHaveBeenLastCalledWith(expect.objectContaining({ require_valid_state: true, scheduling_state_model: 'test-model', scheduling_state_minutes: 50 }))
    await toggle.setValue(false)
    await w.findAll('button').find(b => b.text() === '保存配置')!.trigger('click'); await flushPromises()
    expect(stateKeeperAPI.save).toHaveBeenLastCalledWith(expect.objectContaining({ require_valid_state: false }))
  ...
  it('shows automatic cooldown separately from a manual pause and keeps the saved State accessible', async () => {
    const data = initial()
    Object.assign(data.rows[0], { auto_retry_pending: true, next_retry_at: '2026-09-19T12:00:00Z', retry_reason: '上游 429，账号全部模型冷却后自动重试', hourly_requests: 12, effective_concurrency: 1 })
    vi.mocked(stateKeeperAPI.get).mockResolvedValue(data)
    const w = render(); await flushPromises(); await showModelRows(w)
    expect(w.text()).toContain('冷却中，自动继续')
    expect(w.text()).toContain('上游 429')
  ...
  it('saves per-account concurrency, total budget and degraded lengths together', async () => {
    ...
    await w.findAll('label').find(label => label.text() === '每轮请求上限（全部代理合计）')!.get('input').setValue(9)
