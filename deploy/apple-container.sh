...
STACK_LABEL_KEY="org.qerkai.gateway.stack"
STACK_LABEL_VALUE="apple-container"
NETWORK_NAME="qerkai-gateway-apple"
APP_CONTAINER="qerkai-gateway-apple"
POSTGRES_CONTAINER="qerkai-gateway-apple-postgres"
REDIS_CONTAINER="qerkai-gateway-apple-redis"
APP_VOLUME="qerkai-gateway-apple-data"
POSTGRES_VOLUME="qerkai-gateway-apple-postgres-data"
REDIS_VOLUME="qerkai-gateway-apple-redis-data"
PLATFORM="linux/arm64"

TEMP_DIR=""
LOCK_DIR="${TMPDIR:-/tmp}/qerkai-gateway-apple-container.lock"
LOCK_ACQUIRED=false
...
prepare_environment() {
    ...
    APP_IMAGE="$(read_env_value QERKAI_GATEWAY_IMAGE qerkai-gateway:latest)"
    ...
    TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/qerkai-gateway-apple.XXXXXX")"
...
cmd_logs() {
    ...
    case "${service}" in
        app|qerkai-gateway) container_name="${APP_CONTAINER}" ;;
