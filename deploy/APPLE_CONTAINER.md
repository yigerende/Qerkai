...
```dotenv
QERKAI_GATEWAY_IMAGE=qerkai-gateway:latest
...
| Type | Names |
|---|---|
| Containers | `qerkai-gateway-apple`, `qerkai-gateway-apple-postgres`, `qerkai-gateway-apple-redis` |
| Network | `qerkai-gateway-apple` |
| Volumes | `qerkai-gateway-apple-data`, `qerkai-gateway-apple-postgres-data`, `qerkai-gateway-apple-redis-data` |
...
All three services attach only to the private `qerkai-gateway-apple` network. Only the application publishes a host port; database and Redis ports remain unpublished.

The application container is intentionally recreated by every `up` and `restart` operation because dependency VM addresses can change after they stop. Application data remains in `qerkai-gateway-apple-data`.
...
container exec qerkai-gateway-apple sh -c \
...
container exec qerkai-gateway-apple sh -c 'tar -C "$DATA_DIR" -czf - .' \
...
container delete qerkai-gateway-apple
QERKAI_IMAGE=qerkai-gateway:latest # Match QERKAI_GATEWAY_IMAGE in .env.
container run --rm --name qerkai-gateway-apple-data-restore \
  --entrypoint /bin/sh \
  --volume qerkai-gateway-apple-data:/restore \
...
container start qerkai-gateway-apple-postgres
until container exec qerkai-gateway-apple-postgres sh -c 'pg_isready -U "$POSTGRES_USER" -d "$POSTGRES_DB"'; do sleep 1; done
container copy backups/sub2api.sql qerkai-gateway-apple-postgres:/tmp/sub2api.sql
container exec qerkai-gateway-apple-postgres sh -c '
