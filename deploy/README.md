...
# View logs
docker compose -f docker-compose.local.yml logs -f qerkai-gateway

# If admin password was auto-generated, find it in logs:
docker compose -f docker-compose.local.yml logs qerkai-gateway | grep "admin password"
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs qerkai-gateway | grep "admin password"
   ```
...
docker compose -f docker-compose.local.yml restart qerkai-gateway
...
# View logs
docker compose logs -f qerkai-gateway

# Restart Sub2API only
docker compose restart qerkai-gateway
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 qerkai-gateway
...
# View detailed logs
docker compose logs --tail=100 qerkai-gateway
